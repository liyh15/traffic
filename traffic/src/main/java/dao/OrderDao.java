...
public class OrderDao {
	 ...
	 */
	 public List<Order> getOrderByUserId(int userId)
	 {
		...
		String sql = "select * from tule_order where user_id=?";
		try {
	 ...
	 public void addOrder(Order order)
	 {
		...
		PreparedStatement statement = null;
		ResultSet resultSet = null;
		String sql ="insert into tule_order(user_id,status,traffic_date_arrange_id,totle_price,passenger_id,type,oexplain,reservation,return_price,distribution_address,contact_phone) "
				+ "values(?,?,?,?,?,?,?,?,?,?,?)";
		try {
			connection = DBUtils.getConnection();
			statement = connection.prepareStatement(sql);
